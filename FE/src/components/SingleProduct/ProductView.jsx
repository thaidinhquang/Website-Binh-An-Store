...
const ProductView = ({ className }) => {
  ...
  return (
    <form onSubmit={handleAddToCart} className="p-6 bg-white shadow-lg rounded-lg">
      <div className={`product-view w-full lg:flex justify-between ${className || ""}`}>
        <div data-aos="fade-right" className="lg:w-1/2 xl:mr-[70px] lg:mr-[50px]">
          <div className="w-full grid grid-cols-1 gap-4">
            <div className="w-full h-[600px] border border-qgray-border flex justify-center items-center overflow-hidden relative mb-3 rounded-lg shadow-md">
              <img src={product.image} alt={product.name} className="object-contain w-full h-full" />
              {getStatus(product.createdAt) === 0 && (
                <div className="w-[80px] h-[80px] rounded-full bg-qyellow text-qblack flex justify-center items-center text-xl font-medium absolute left-[30px] top-[30px] shadow-lg">
                  New
                </div>
              )}
            </div>
            ...
            {product.images && product.images.map((img, index) => (
              <div key={index} className="w-full h-[150px] border border-qgray-border flex justify-center items-center overflow-hidden relative mb-3 rounded-lg shadow-md">
                <img src={img} alt={`${product.name} ${index}`} className="object-contain w-full h-full" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1">
          <div className="product-details w-full mt-10 lg:mt-0">
            <p data-aos="fade-up" className="text-3xl font-bold text-qblack mb-4">
              {product.name}
            </p>

            <div data-aos="fade-up" className="flex space-x-2 items-center mb-7">
              <span className="text-lg font-semibold text-qgray line-through mt-2">
              ...
              {product?.attributes.map(attribute => (
                <div key={attribute._id} className="mr-4 mb-4">
                  <p
                    ...
                    className={`cursor-pointer btn ${selectedAttribute === attribute._id ? 'bg-blue-500 text-white' : 'bg-gray-200 text-black'} px-4 py-2 rounded-md shadow-sm transition duration-300 ease-in-out transform hover:bg-blue-600 hover:text-white`}
                  >
                    {attribute.name}
                  </p>
                </div>
              ...
              {product?.attributes.map(attribute => (
                selectedAttribute === attribute._id && attribute.values.map(value => (
                  <button
                    ...
                    className={`px-4 py-2 border border-qgray-border mb-2 bg-gray-100 hover:bg-blue-100 rounded-md shadow-sm transition duration-300 ease-in-out transform hover:scale-105 ${selectedAttributes[attribute._id]?._id === value._id ? 'bg-blue-200' : ''}`}
                  >
            ...
            <div data-aos="fade-up" className="quantity-card-wrapper w-full flex items-center h-[50px] space-x-[10px] mb-[30px]">
              <div className="w-[120px] h-full px-[26px] flex items-center border border-qgray-border rounded-md shadow-sm">
                <div className="flex justify-between items-center w-full">
                  <button onClick={handleDecrement} type="button" className="text-base text-qgray transition duration-300 ease-in-out transform hover:scale-105 hover:bg-gray-300">
                    -
                  </button>
                  <span className="text-qblack">{quantity}</span>
                  <button onClick={handleIncrement} type="button" className="text-base text-qgray transition duration-300 ease-in-out transform hover:scale-105 hover:bg-gray-300">
                    +
                  </button>
                </div>
              </div>

              <div className="w-[60px] h-full flex justify-center items-center border border-qgray-border rounded-md shadow-sm">
                <button type="button" className="transition duration-300 ease-in-out transform hover:scale-105 hover:bg-gray-300">
                  <span>
              ...
              <div className="flex-1 h-full">
                <button type="submit" className="black-btn text-sm font-semibold w-full h-full bg-blue-500 text-white rounded-md shadow-lg transition duration-300 ease-in-out transform hover:bg-blue-600">
                  Add To Cart
                </button>
              </div>
            </div>
