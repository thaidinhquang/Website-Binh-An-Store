...
const ProductList = () => {
  ...
  console.log("Search parameters:", { active, page, sort, query });
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off('userEditing', handleUserEditing);
    };
  }, []);

  useEffect(() => {
    console.log("Refetching data with parameters:", { active, page, sort, query });
    refetch();
  }, [active, page, sort, query]);

  const searchForm = (data) => {
    const params = new URLSearchParams(data).toString();
    console.log("Navigating to:", `/admin/products?${params}`);
  ...
  if (!data) return <div className="flex justify-center items-center h-screen"><div>Loading...</div></div>;

  // Kiểm tra nếu không có sản phẩm nào khớp với tìm kiếm
  if (data.docs.length === 0) return <div className="flex justify-center items-center h-screen"><div>Không tìm thấy sản phẩm nào</div></div>;
                  ...
                  <tr key={product._id}>
                    ...
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <div className="flex items-center space-x-4 text-sm">
                        <Link to={`/admin/products/detail/${product._id}`} className="text-blue-600 hover:text-blue-900">
                          Chi tiết
                        </Link>
                        <Link to={`/admin/products/edit/${product._id}`} className="text-yellow-600 hover:text-yellow-900">
                          Sửa
                        </Link>
                      </div>
                    </td>
